...
//
// DATA SOURCES (all live, current-minute data only):
//   Equities, yields, FX, commodities → Yahoo Finance via proxy chain
//   Crypto prices + dominance          → CoinGecko direct (no key needed)
//   Crypto backup                      → Binance direct   (no key needed)
//   FX rates                           → Frankfurter.app direct (no key)
//   FX backup                          → open.er-api.com  (no key needed)
//   News headlines                     → Reuters RSS / BBC RSS via proxy
//
// REMOVED (stale data — not useful for real-time dashboard):
//   World Bank CPI / lending rate (1-2 year lag — removed)
//   Econdb macro calendar (proxy fails consistently — removed)
// ================================================================
...
// Parse Yahoo Finance chart API — extracts LIVE price correctly
// The chart endpoint returns:
//   meta.regularMarketPrice  → current live/last-trade price ✓
//   meta.previousClose       → previous session close ✓
//   meta.regularMarketTime   → unix timestamp of last trade ✓
// We NEVER use chartPreviousClose (that's the close before previous close)
function parseYahoo(json) {
  ...
  // regularMarketPrice = most recent trade price (live during market hours,
  // last close when market is closed) — this is always the correct current price
  const price = meta.regularMarketPrice;
  if (!price || price === 0) return null;

  // previousClose = the close of the most recently completed session
  ...
  // Timestamp of last trade — used to detect truly stale data
  const lastTradeTs = meta.regularMarketTime
    ? meta.regularMarketTime * 1000
    : null;

  // Flag as stale if last trade was more than 36 hours ago
  // (covers weekends — last trade Friday is valid on Saturday/Sunday)
  const isStale = lastTradeTs
    ? (Date.now() - lastTradeTs) > 36 * 60 * 60 * 1000
    : false;
...
async function fetchYahooSymbol(sym) {
  // Use interval=2m range=1d → returns intraday bars with live last price
  // interval=1d range=1d only returns the daily bar which can be yesterday's close
  // For yields/FX/commodities use 5m interval which updates more frequently
  const isYield = sym.startsWith('^I') || sym === '^IRX' || sym === '^FVX' || sym === '^TNX' || sym === '^TYX';
  const interval = isYield ? '5m' : '2m';

  const endpoints = [
    `https://query2.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(sym)}?interval=${interval}&range=1d&includePrePost=true`,
    `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(sym)}?interval=${interval}&range=1d&includePrePost=true`,
    // Fallback: daily interval (less fresh but works when 2m is blocked)
    ...
    for (const mkProxy of proxies) {
      ...
      try {
        ...
        if (result && !result.stale) {
          // Fresh data — save to cache and return
          CACHE.set('y_'+sym, result);
          return result;
        } else if (result && result.stale) {
          // Got data but it's stale (weekend/holiday) — still valid, save it
          CACHE.set('y_'+sym, result);
          return result;
        }
      } catch(e) { continue; }
  ...
  // All proxies failed — return cache
  const cached = CACHE.get('y_'+sym, 48*60*60*1000); // accept up to 48h cache
...
// ── FX: Frankfurter (live, updates every minute) → open.er-api fallback
// Frankfurter pulls from ECB — updates every business minute
// Far more reliable and fresher than Yahoo Finance FX via proxy
async function fetchFX() {
  // Frankfurter gives rates relative to USD
  try {
    const r = await fetchRaw(
      `https://api.frankfurter.app/latest?from=USD&to=EUR,GBP,JPY,INR,CNY,AUD,CAD`,
      6000
    );
    const d = await r.json();
    if (d?.rates) {
      // Also fetch DXY proxy: USD vs basket of EUR,GBP,JPY,CAD,SEK,CHF
      // Frankfurter doesn't have DXY directly but we can track EUR/USD
      const rates = {
        EUR: d.rates.EUR,   // EUR/USD = 1/d.rates.EUR
        GBP: d.rates.GBP,
        JPY: d.rates.JPY,
        INR: d.rates.INR,
        CNY: d.rates.CNY,
        ...
        EURUSD: d.rates.EUR ? 1/d.rates.EUR : null,
        GBPUSD: d.rates.GBP ? 1/d.rates.GBP : null,
        USDJPY: d.rates.JPY,
        USDINR: d.rates.INR,
        ts: Date.now(),
      };
      CACHE.set('fx_live', rates);
      return rates;
    }
  } catch(e) {}
  ...
  try {
    const r = await fetchRaw('https://open.er-api.com/v6/latest/USD', 6000);
    const d = await r.json();
    if (d?.rates) {
      const rates = {
        EUR: d.rates.EUR, GBP: d.rates.GBP, JPY: d.rates.JPY,
        INR: d.rates.INR, CNY: d.rates.CNY, AUD: d.rates.AUD, CAD: d.rates.CAD,
        EURUSD: d.rates.EUR ? 1/d.rates.EUR : null,
        GBPUSD: d.rates.GBP ? 1/d.rates.GBP : null,
        USDJPY: d.rates.JPY, USDINR: d.rates.INR,
        ts: Date.now(),
      };
      CACHE.set('fx_live', rates);
      return rates;
  ...
  return CACHE.get('fx_live', 6*60*60*1000) || {};
}

// ── NEWS: Yahoo Finance → Reuters → BBC → CNBC ───────────────────────
// Rotates proxies on each call to avoid cached responses.
// newsCallCount increments each refresh to ensure different proxy is used.
let newsCallCount = 0;
async function fetchNews() {
  newsCallCount++;
  const ts = Date.now();

  // Proxy builders — we rotate which one is FIRST each call
  // so allorigins doesn't keep serving the same cached response
  const ALL_PROXIES = [
  ...
  // Rotate starting proxy based on call count
  const proxies = [
    ...ALL_PROXIES.slice(newsCallCount % ALL_PROXIES.length),
    ...ALL_PROXIES.slice(0, newsCallCount % ALL_PROXIES.length),
  ];

  // Helper: parse allorigins wrapped response
  const parseProxy = async (r, proxiedUrl) => {
    if (proxiedUrl.includes('allorigins')) {
      const w = await r.json();
      return w?.contents ? w.contents : null;
    }
    return await r.text();
  };

  // ── ATTEMPT 1: Yahoo Finance News API ──────────────────────────────
  const YF_NEWS_QUERIES = ['markets', 'stocks economy', 'federal reserve inflation'];
  const query = YF_NEWS_QUERIES[newsCallCount % YF_NEWS_QUERIES.length];
  const yfUrl = `https://query1.finance.yahoo.com/v1/finance/search?q=${encodeURIComponent(query)}&newsCount=8&quotesCount=0&enableNavLinks=false`;

  for (const mkProxy of proxies) {
    const proxied = mkProxy(yfUrl);
    try {
      const r    = await fetchRaw(proxied, 7000);
      const text = await parseProxy(r, proxied);
      if (!text) continue;
      const json = JSON.parse(text);
      const news = json?.news || [];
      if (news.length > 0) {
        const articles = news.slice(0, 8).map(n => ({
          title:   n.title || '',
  ...
  // ── ATTEMPT 2: Yahoo Finance v2 trending news ───────────────────────
  const yfV2 = `https://query2.finance.yahoo.com/v1/finance/trending/US?count=10&ts=${ts}`;
  for (const mkProxy of proxies) {
    const proxied = mkProxy(yfV2);
    try {
      const r    = await fetchRaw(proxied, 7000);
      const text = await parseProxy(r, proxied);
      if (!text) continue;
      ...
      // trending gives symbols not news, skip to RSS
      if (quotes.length === 0) continue;
    } catch(e) { continue; }
    break;
  }

  // ── ATTEMPT 3: RSS feeds — try each proxy for each source ──────────
  const RSS = [
  ...
  // All sources failed — serve cache
...
function populateQS() {
  ...
  // Supplement FX from Frankfurter live rates (more accurate than Yahoo FX via proxy)
  if (FX.USDINR) { QS.inrPrice = FX.USDINR; }
  if (FX.EURUSD && !GD['EURUSD=X']) {
    GD['EURUSD=X'] = { price: FX.EURUSD, changePct: 0, change: 0 };
  }
...
function renderMarketRow(sym) {
  ...
  // Show when data was last traded
  let timeLabel = '';
  if (d?.lastTradeTs) {
    const diff = (Date.now() - d.lastTradeTs) / 60000; // minutes ago
    if (diff < 2)        timeLabel = 'live';
    else if (diff < 60)  timeLabel = Math.round(diff) + 'm ago';
    else if (diff < 1440)timeLabel = Math.round(diff/60) + 'h ago';
    else                 timeLabel = Math.round(diff/1440) + 'd ago';
  }

  const staleTag = d?.stale
    ? '<span style="font-size:9px;color:var(--gold);margin-left:4px">CACHED</span>'
    : (timeLabel ? `<span style="font-size:9px;color:var(--txt-3);margin-left:4px">${timeLabel}</span>` : '');

  return `<div class="mkt-row" title="${NAMES[sym]||sym}: ${d?fmt(d.price,2):'N/A'} (${fmtPct(pct)})">
    <div>
      <div class="mkt-name">${NAMES[sym]||sym}${staleTag}</div>
      <div class="mkt-tick">${sym}</div>
    </div>
...
function renderFX() {
  const pairs=[
    {sym:'EURUSD=X', label:'EUR/USD',   sub:'Euro',         fxKey:'EURUSD'},
    {sym:'GBPUSD=X', label:'GBP/USD',   sub:'Cable',        fxKey:'GBPUSD'},
    {sym:'USDJPY=X', label:'USD/JPY',   sub:'Yen',          fxKey:'USDJPY'},
    {sym:'DX-Y.NYB', label:'DXY Index', sub:'Dollar Index',  fxKey:null   },
    {sym:'USDINR=X', label:'USD/INR',   sub:'Rupee',        fxKey:'USDINR'},
  ];
  const el=$('fxTable'); if(!el) return;
  el.innerHTML=pairs.map(f=>{
    const yahooD=GD[f.sym];
    // Frankfurter live price is primary (updates every minute from ECB)
    const price = (f.fxKey && FX[f.fxKey]) ? FX[f.fxKey] : yahooD?.price;
    const pct   = yahooD?.changePct || 0;
    const isLive= !!(f.fxKey && FX[f.fxKey]);
    const liveTag = isLive
      ? ' <span style="font-size:9px;color:var(--green)">LIVE</span>'
      : (yahooD?.stale ? ' <span style="font-size:9px;color:var(--gold)">CACHED</span>' : '');
    const gauge=Math.min(100,Math.abs(pct)*30+30);
    return `<div class="rate-row">
      <div><div class="rate-name">${f.label}${liveTag}</div><div class="rate-sub">${f.sub}</div></div>
      <div class="rate-bar"><div class="rate-bar-fill" style="width:${gauge}%"></div></div>
      <div class="rate-val">${price?fmt(price,4):'—'}</div>
      <div class="rate-chg ${clr(pct)}">${fmtPct(pct)}</div>
    </div>`;
  }).join('');
}
...
function renderNews(articles) {
  const CATS=[
    {cat:'nc-fed',   label:'CENTRAL BANK',keys:['fed','fomc','powell','rate cut','rate hike','federal reserve','ecb','rbi','boe','boj','central bank','monetary policy','interest rate','hawkish','dovish']},
    {cat:'nc-ai',    label:'AI / TECH',   keys:['nvidia','ai','artificial intelligence','semiconductor','chip','tsmc','openai','amd','microsoft','google','meta','apple','amazon','tech']},
    {cat:'nc-macro', label:'MACRO',       keys:['cpi','inflation','gdp','jobs','nfp','pmi','ism','recession','retail sales','ppi','unemployment','economy','growth']},
    {cat:'nc-geo',   label:'GEO',         keys:['china','russia','ukraine','war','sanction','tariff','trade','nato','iran','opec','geopolit','conflict']},
    {cat:'nc-energy',label:'ENERGY',      keys:['oil','crude','brent','wti','opec','natural gas','energy','petroleum','barrel']},
  ];
  ...
  const sentiment= t => { const tl=t.toLowerCase(); const b=['surge','rally','rise','gain','beat','strong','record','cut rate','soar','jump','boost'].filter(w=>tl.includes(w)).length; const r=['fall','drop','crash','decline','miss','weak','recession','war','tighten','slump','plunge'].filter(w=>tl.includes(w)).length; return b>r?'bullish':r>b?'bearish':'neutral'; };
  const ago = d => {
    const diff = (Date.now()-new Date(d).getTime())/60000;
    if (isNaN(diff)||diff<0) return 'just now';
    return diff<60?Math.round(diff)+'m ago':diff<1440?Math.round(diff/60)+'h ago':Math.round(diff/1440)+'d ago';
  };

  const el=$('newsGrid'); if(!el) return;
  if(!articles||!articles.length){
    el.innerHTML=`<div class="news-item" style="grid-column:1/-1;text-align:center;padding:24px;color:var(--txt-3);font-family:var(--f-mono);font-size:11px;">News unavailable — all sources failed. Will retry on next refresh.</div>`;
    const ts=$('newsTimestamp'); if(ts) ts.textContent='Failed — retrying in 45s';
    return;
  }
  el.innerHTML=articles.slice(0,6).map(a=>{
    ...
    const link=a.link&&a.link!=='#'?`href="${a.link}" target="_blank" rel="noopener"`:'';
    return `<div class="news-item" style="cursor:${link?'pointer':'default'}" ${link?`onclick="window.open('${a.link}','_blank')"`:''}><div class="news-hdr"><span class="news-cat ${cat.cat}">${cat.label}</span><span class="news-time">${ago(a.pubDate)}</span></div><div class="news-hl">${a.title}</div><div class="news-imp ${imp}">→ ${desc||'Click to read full article.'}${desc.length>=130?'...':''}</div><div class="news-src">${a.source}</div></div>`;
  }).join('');
  const ts=$('newsTimestamp');
  if(ts) ts.textContent='Updated '+new Date().toLocaleTimeString()+' (fetch #'+newsCallCount+')';
}
...
function drawYieldCurve(labels,vals){
  ...
  if(yChart){
    yChart.data.datasets[0].data=valid;
    yChart.data.datasets[0].borderColor=color;
    yChart.data.datasets[0].backgroundColor=color+'22';
    yChart.update('none');
  } else {
    yChart=new Chart(ctx,{
      ...
      options:{
        ...
        scales:{
          x:{grid:{color:'rgba(255,255,255,.04)'},ticks:{color:'#555568',font:{family:'Space Mono',size:10}}},
          y:{grid:{color:'rgba(255,255,255,.04)'},ticks:{color:'#555568',font:{family:'Space Mono',size:10},callback:v=>v?.toFixed(2)+'%'}}
        }
      }
    });
  }
...
async function refreshAll(){
  ...
  try {
    ...
    const dot    = fresh > 30 ? '🟢' : fresh > 15 ? '🟡' : '🔴';
    $('lastRefresh').textContent = `${dot} ${fresh} live, ${stale} cached — ${new Date().toLocaleTimeString()}`;

    startCountdown();

    // Background: news always fetches fresh (non-blocking)
