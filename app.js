...
// ── NEWS: Yahoo Finance → Reuters RSS → BBC RSS fallback ─────────────
// Yahoo Finance news is financial-specific and updates in real time.
// Same proxy chain used for price data works here too.
// Cache-busted on every call so news is always the latest batch.
async function fetchNews() {
  const ts = Date.now(); // cache-buster — forces fresh fetch every time

  // ── ATTEMPT 1: Yahoo Finance Search News API ──────────────────────
  // Returns top financial news headlines with descriptions and sources
  ...
  for (const q of YAHOO_NEWS_QUERIES) {
    const yahooUrl = `https://query1.finance.yahoo.com/v1/finance/search?q=${encodeURIComponent(q)}&newsCount=6&enableNavLinks=false&quotesCount=0`;
    const proxies = shuffle([
      u => `https://corsproxy.io/?${encodeURIComponent(u)}`,
      u => `https://api.allorigins.win/get?url=${encodeURIComponent(u)}&_=${ts}`,
      u => `https://corsproxy.org/?${encodeURIComponent(u)}`,
    ]);

    for (const mkProxy of proxies) {
      try {
        const r = await fetchRaw(mkProxy(yahooUrl), 7000);
        let json;
        if (mkProxy(yahooUrl).includes('allorigins')) {
          const w = await r.json();
          if (!w?.contents) continue;
          json = JSON.parse(w.contents);
        } else {
        ...
        const news = json?.news || json?.finance?.result?.[0]?.news || [];
        if (news.length > 0) {
  ...
  // ── ATTEMPT 2: Yahoo Finance symbol news (market movers) ─────────
  // Fetches news attached to major market symbols — very fresh
  const SYMBOL_NEWS_SYMS = ['^GSPC', 'NVDA', 'AAPL'];
  for (const sym of SYMBOL_NEWS_SYMS) {
    const yahooUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${sym}?interval=1d&range=1d&includePrePost=false&events=div%2Csplit`;
    const proxies = shuffle([
      u => `https://corsproxy.io/?${encodeURIComponent(u)}`,
      u => `https://api.allorigins.win/get?url=${encodeURIComponent(u)}&_=${ts}`,
    ]);
    // This endpoint sometimes embeds news in the payload — try next if not
    // (We already fetch prices from here; news is a bonus)
    // Skip this and go to RSS if search API failed
    break;
  }

  // ── ATTEMPT 3: Reuters Business RSS ──────────────────────────────
  const RSS_SOURCES = [
  ...
  for (const src of RSS_SOURCES) {
    try {
      const proxied = `https://api.allorigins.win/get?url=${encodeURIComponent(src.url)}&_=${ts}`;
  ...
  // All sources failed — serve cache only as absolute last resort
...
function renderNews(articles) {
  ...
  const ts=$('newsTimestamp'); if(ts) ts.textContent='Updated '+new Date().toLocaleTimeString();
}
