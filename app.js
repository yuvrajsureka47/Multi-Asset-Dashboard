...
// ── NEWS: Always fetches fresh on every call — no cache served ────────
async function fetchNews() {
  ...
  // Cache-bust with timestamp so proxy doesn't serve stale response
  const ts = Date.now();
  const proxyUrl = u => `https://api.allorigins.win/get?url=${encodeURIComponent(u)}&_=${ts}`;
  ...
  for (const src of sources) {
    try {
      ...
      // Only save to cache as backup — never read from cache as primary
      CACHE.set('news', articles);
      return articles;
    } catch(e) { continue; }
  }
  // All sources failed — serve cache only as last resort
